	  ...
	  if(DHT22_Start())
	     {
	         ...
	         RH2 = DHT22_Read(); // Second 8 bits of relative humidity
	         TC1 = DHT22_Read(); // First 8 bits of Celsius
	         TC2 = DHT22_Read(); // Second 8 bits of Celsius
	         SUM = DHT22_Read(); // Checksum
	   ...
	   // Create a buffer to hold the formatted temperature string
	  char tempStr[100];  // Increased buffer size to hold temperature and humidity
	  sprintf(tempStr, "Temp: %.2f C, %.2f F, Humidity: %.2f%%\r\n", tCelsius, tFahrenheit, RH);

	  // Transmit the temperature and humidity string over UART
